...
var calcWeightedAvg = function(values, weightType) {
  ...
  if (n > 0) {
    var weightedSum = weights.reduce(function(a, b) {
      return a + b;
    });
...
var analyzeT = function(perfData) {
  return new Promise(function(resolve, reject) {
    ...
    goodData.forEach(function(entry, index) {
      ...
      var prev = goodData[index - 1];

      if (prev.t > entry.t) {
        return;
      }

      var next = goodData[index + 1];
      ...
      .then(function(results) {
        ...
        results.forEach(function(result, index) {
          if (result.state === 'regression') {
            var prevAvg = result.historicalStats;

            regressionList.push(result);
          }
        });
